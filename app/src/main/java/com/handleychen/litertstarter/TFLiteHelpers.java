...
public class TFLiteHelpers {
    ...
    public static Pair<Interpreter, Map<DelegateType, Delegate>> CreateInterpreterAndDelegatesFromOptions(
        ...
        for (DelegateType[] delegatesToRegister : delegatePriorityOrder) {
            ...
            if (Arrays.stream(delegatesToRegister).anyMatch(x -> !delegates.containsKey(x))) {
                Log.w(TAG, "CreateInterpreterAndDelegatesFromOptions() Unable to create all delegates for this attempt. Skipping.");
                continue;
            }
            ...
            if (interpreter == null) {
                Log.w(TAG, "CreateInterpreterAndDelegatesFromOptions() Unable to create interpreter for this attempt. Skipping.");
                continue;
            }
            List<String> delegateStringList = Arrays.stream(delegatesToRegister).map(Enum::name).collect(Collectors.toCollection(ArrayList<String>::new));
            Log.i(TAG, "CreateInterpreterAndDelegatesFromOptions() Successfully created interpreter & delegates. {" + String.join(", ", delegateStringList) + "}");
            ...
            delegates.keySet().stream()
                    ...
                    .forEach(unusedDelegateType -> {
                        Log.w(TAG, "CreateInterpreterAndDelegatesFromOptions() remove unused delegate " + unusedDelegateType.name() + "");
    ...
    static Delegate CreateQNN_NPUDelegate(String nativeLibraryDir, String cacheDir, String modelIdentifier) {
        ...
        //qnnOptions.setLogLevel(LogLevel.LOG_LEVEL_WARN);
